...
def compare_models(prompt: str) -> dict:
    ...
    """
    # Call GPT-4o
    gpt4o_text, gpt4o_lat, gpt4o_usage = call_openai(
...
def streaming_chatbot() -> None:
    ...
    if not api_key:
        print(
            "\033[93m[System Warning] GEMINI_API_KEY environment variable not set. Running in dummy mode.\033[0m")
        api_key = "mock-key"
    ...
    while True:
        try:
            ...
            try:
                ...
                response_stream = model_inst.generate_content(
                    legacy_contents, stream=True)
                for chunk in response_stream:
                    chunk_text = chunk.text or ""
                    print(chunk_text, end="", flush=True)
                    full_reply += chunk_text
    ...
    for r in results:
        ...
        for name, data in models:
            ...
            if isinstance(data, dict):
                ...
                resp = str(data)
                lat = r.get("gpt4o_latency" if name ==
                            "GPT-4o" else "mini_latency", 0.0)
                in_t = int(len(prompt.split()) * 1.5)
                out_t = int(len(resp.split()) * 1.5)
                cost = r.get("gpt4o_cost_estimate" if name ==
                             "GPT-4o" else "gpt4o_cost_estimate", 0.0)
